...
class SoftBoard(object):
    ...
    def check_vulnerability(self, x, y):
        ...
        vul_cond = lambda x1, y1, x2, y2, x3, y3: (
                        ...
                        stone == _at(self.content, x2, y2) and
                        _at(self.content, x1, y1) != stone and _at(self.content, x3, y3) != stone and
                        _at(self.content, x1, y1) != _at(self.content, x3, y3) and
                        (_at(self.content, x1, y1) == STONE_EMPTY or _at(self.content, x3, y3) == STONE_EMPTY))
        vul_tab = (
            (x-1, y, x+1, y, x+2, y),
            (x+1, y, x-1, y, x-2, y),
            (x, y-1, x, y+1, x, y+2),
    ...
    def check_destroyable(self, x, y, stone):
        ...
        destroy_cond = lambda x1, y1, x2, y2, x3, y3: (
                        0 <= x1 < G.GET("BOARD_SZ") and 0 <= x2 < G.GET("BOARD_SZ") and 0 <= x3 < G.GET("BOARD_SZ") and
                        0 <= y1 < G.GET("BOARD_SZ") and 0 <= y2 < G.GET("BOARD_SZ") and 0 <= y3 < G.GET("BOARD_SZ") and
                        _at(self.content, x3, y3) == stone and
                        _at(self.content, x2, y2) == _at(self.content, x1, y1) and
                        _at(self.content, x1, y1) not in (STONE_EMPTY, stone))
    ...
    def _check_aligned_dir(self, x, y, stone, addx, addy, check_only=False):
        ...
        while 0 <= new_x < G.GET("BOARD_SZ") and 0 <= new_y < G.GET("BOARD_SZ"):
            if _at(self.content, new_x, new_y) == stone:
                ...
                while 0 <= new_x < G.GET("BOARD_SZ") and 0 <= new_y < G.GET("BOARD_SZ"):
                    if _at(self.content, new_x, new_y) == stone:
                        if not self.softMode and \
                           (self.is_vulnerable_victory[stone-1] or nb_aligned_non_vulnerable >= G.GET("NB_ALIGNED_VICTORY")):
    ...
    def check_aligned(self, x, y, check_only=False):
        ...
        if total_is_aligned:
            if not check_only:
                if self.is_vulnerable_victory[stone-1] or total_is_not_vulnerable:
                    self.game.players[stone-1].is_win_aligned = True
    ...
    def check_winner(self):
        ...
        for x in range(G.GET("BOARD_SZ")):
            for y in range(G.GET("BOARD_SZ")):
                if _at(self.content, x, y) != STONE_EMPTY:
                    self.nb_total_stones += 1
                    self.game.players[_at(self.content, x, y) - 1].nb_stone += 1
                self.check_vulnerability(x, y)

        tmp_is_vulnerable_victory = [False for i in range(len(self.game.players))]
        for x in range(G.GET("BOARD_SZ")):
            for y in range(G.GET("BOARD_SZ")):
                tmp_is_vulnerable_victory[_at(self.content, x, y) - 1] = tmp_is_vulnerable_victory[_at(self.content, x, y) - 1] or self.check_aligned(x, y)
    ...
    def put_stone(self, x, y, stone, test=False):
        ...
        _set(self.content, x, y, stone)

        # destroy some stones if needed
        destroyed = self.check_destroyable(x, y, stone)
        for dest_x, dest_y in destroyed:
            _set(self.content, dest_x, dest_y, STONE_EMPTY)
            self.remain_places += 1
            if not test:
                self.game.players[stone - 1].destroyed_stones_count += 1
    ...
    def _is_free_three_dir(self, x, y, stone, addx, addy):
        ...
        while i < len_free_three:
            if 0 <= new_x < G.GET("BOARD_SZ") and 0 <= new_y < G.GET("BOARD_SZ"):
                lst[i] = _at(self.content, new_x, new_y)
            new_x += addx
            new_y += addy
            i += 1
    ...
    def is_allowed(self, x, y, stone):
        ...
        if nb_free_three >= 2:
            ...
            _set(self.content, x, y, STONE_EMPTY)
            if check_aligned:
                return True
            else:
    ...
    def __str__(self):
        ...
        for y in range(G.GET("BOARD_SZ")):
            ...
            for x in range(G.GET("BOARD_SZ")):
                ...
                if _at(self.content, x, y) == STONE_EMPTY:
                    pass
                elif _at(self.content, x, y) == 0:
                    color = [c.CYAN, c.F_CYAN]
                elif _at(self.content, x, y) == 1:
                    color = [c.WHITE, c.F_WHITE]
                elif _at(self.content, x, y) == 2:
                    color = [c.RED, c.F_RED]
                elif _at(self.content, x, y) == 3:
                    color = [c.GREEN, c.F_GREEN]
                elif _at(self.content, x, y) == 4:
                    color = [c.BLUE, c.F_BLUE]
                elif _at(self.content, x, y) == 5:
                    color = [c.YELLOW, c.F_YELLOW]
                elif _at(self.content, x, y) == 6:
                    color = [c.MAGENTA, c.F_MAGENTA]
                elif _at(self.content, x, y) == 7:
                    color = [c.BLACK, c.F_BLACK]
                else:
                    txt = '%2d ' % (_at(self.content, x, y))
